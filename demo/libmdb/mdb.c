...
#define PAGEFILL(env, p)                                              \
	(1000 * ((env)->me_head.mh_psize - PAGEHDRSZ - SIZELEFT(p)) / \
...
static MDB_dpage *mdb_newpage(MDB_txn *txn, MDB_page *parent, int parent_idx,
			      int num);
...
static int mdb_search_page_root(MDB_db *db, MDB_val *key, MDB_cursor *cursor,
				int modify, MDB_pageparent *mpp);
static int mdb_search_page(MDB_db *db, MDB_txn *txn, MDB_val *key,
...
static int mdbenv_write_meta(MDB_env *env, pgno_t root, unsigned int flags);
static MDB_page *mdbenv_get_page(MDB_env *env, pgno_t pgno);

static MDB_node *mdb_search_node(MDB_db *db, MDB_page *mp, MDB_val *key,
				 int *exactp, unsigned int *kip);
static int mdb_add_node(MDB_db *bt, MDB_page *mp, indx_t indx, MDB_val *key,
			MDB_val *data, pgno_t pgno, uint8_t flags);
static void mdb_del_node(MDB_db *bt, MDB_page *mp, indx_t indx);
static int mdb_read_data(MDB_db *bt, MDB_page *mp, MDB_node *leaf,
			 MDB_val *data);

static int mdb_rebalance(MDB_db *bt, MDB_pageparent *mp);
static int mdb_update_key(MDB_db *bt, MDB_page *mp, indx_t indx, MDB_val *key);
static int mdb_move_node(MDB_db *bt, MDB_pageparent *src, indx_t srcindx,
			 MDB_pageparent *dst, indx_t dstindx);
static int mdb_merge(MDB_db *bt, MDB_pageparent *src, MDB_pageparent *dst);
static int mdb_split(MDB_db *bt, MDB_page **mpp, unsigned int *newindxp,
...
static MDB_ppage *cursor_push_page(MDB_cursor *cursor, MDB_page *mp);

static int mdb_set_key(MDB_db *bt, MDB_page *mp, MDB_node *node, MDB_val *key);
...
static int mdb_cursor_set(MDB_cursor *cursor, MDB_val *key, MDB_val *data,
			  int *exactp);
static int mdb_cursor_first(MDB_cursor *cursor, MDB_val *key, MDB_val *data);

static size_t mdb_leaf_size(MDB_db *bt, MDB_val *key, MDB_val *data);
static size_t mdb_branch_size(MDB_db *bt, MDB_val *key);

static pgno_t mdbenv_compact_tree(MDB_env *env, pgno_t pgno, MDB_env *envc);
...
static int _mdb_cmp(MDB_db *db, const MDB_val *key1, const MDB_val *key2)
{
	if (F_ISSET(db->md_flags, MDB_REVERSEKEY))
		return memnrcmp(key1->mv_data, key1->mv_size, key2->mv_data,
				key2->mv_size);
	else
		return memncmp((char *)key1->mv_data, key1->mv_size,
			       key2->mv_data, key2->mv_size);
}
			      ...
			      int num)
{
	...
	if ((dp = malloc(txn->mt_env->me_head.mh_psize * num +
			 sizeof(MDB_dhead))) == NULL)
		return NULL;
	dp->h.md_num = num;
...
static int mdb_touch(MDB_txn *txn, MDB_pageparent *pp)
{
	...
	if (!F_ISSET(mp->mp_flags, P_DIRTY)) {
		MDB_dpage *dp;
		DPRINTF("touching page %lu -> %lu", mp->mp_pgno,
			txn->mt_next_pgno);
		if ((dp = mdb_newpage(txn, pp->mp_parent, pp->mp_pi, 1)) ==
		    NULL)
			return ENOMEM;
		pgno = dp->p.mp_pgno;
		...
		if (pp->mp_parent != NULL)
			NODEPGNO(NODEPTR(pp->mp_parent, pp->mp_pi)) =
				mp->mp_pgno;
		pp->mp_page = mp;
	}
	return 0;
...
int mdb_txn_begin(MDB_env *env, int rdonly, MDB_txn **ret)
{
	...
	DPRINTF("begin transaction on mdbenv %p, root page %lu", env,
		txn->mt_root);
...
void mdb_txn_abort(MDB_txn *txn)
{
	...
	env = txn->mt_env;
	DPRINTF("abort transaction on mdbenv %p, root page %lu", env,
		txn->mt_root);
...
int mdb_txn_commit(MDB_txn *txn)
{
	...
	DPRINTF("committing transaction on mdbenv %p, root page %lu", env,
		txn->mt_root);
...
int mdbenv_open2(MDB_env *env, unsigned int flags)
{
	...
	if (env->me_head.mh_address && (flags & MDB_FIXEDMAP))
		i |= MAP_FIXED;
	env->me_map = mmap(env->me_head.mh_address, env->me_mapsize, PROT_READ,
			   i, env->me_fd, 0);
 ...
 */
static MDB_node *mdb_search_node(MDB_db *bt, MDB_page *mp, MDB_val *key,
				 int *exactp, unsigned int *kip)
{
	unsigned int i = 0;
	int low, high;
	...
	DPRINTF("searching %lu keys in %s page %lu", NUMKEYS(mp),
	...
	while (low <= high) {
		...
		if (IS_LEAF(mp))
			DPRINTF("found leaf index %u [%.*s], rc = %i", i,
				(int)nodekey.mv_size, (char *)nodekey.mv_data,
				rc);
		else
...
static int mdb_search_page_root(MDB_db *bt, MDB_val *key, MDB_cursor *cursor,
				int modify, MDB_pageparent *mpp)
{
	...
	while (IS_BRANCH(mp)) {
		...
		DPRINTF("branch page %lu has %lu keys", mp->mp_pgno,
			NUMKEYS(mp));
		assert(NUMKEYS(mp) > 1);
		...
		if (key)
			DPRINTF("following index %u for key %.*s", i,
...
static int mdb_read_data(MDB_db *db, MDB_page *mp, MDB_node *leaf,
			 MDB_val *data)
{
...
static int mdb_sibling(MDB_cursor *cursor, int move_right)
{
	...
	DPRINTF("parent page is page %lu, index %u", parent->mp_page->mp_pgno,
		parent->mp_ki);
...
static int mdb_cursor_next(MDB_cursor *cursor, MDB_val *key, MDB_val *data)
{
	...
	DPRINTF("cursor_next: top page is %lu in cursor %p", mp->mp_pgno,
		cursor);
	...
	if (top->mp_ki + 1 >= NUMKEYS(mp)) {
		...
		top = CURSOR_TOP(cursor);
		mp = top->mp_page;
		DPRINTF("next page is %lu, key index %u", mp->mp_pgno,
			top->mp_ki);
	} else
		top->mp_ki++;
			  ...
			  int *exactp)
{
	...
	rc = mdb_search_page(cursor->mc_db, cursor->mc_txn, key, cursor, 0,
			     &mpp);
	if (rc != MDB_SUCCESS)
		return rc;
	...
	top = CURSOR_TOP(cursor);
	leaf = mdb_search_node(cursor->mc_db, mpp.mp_page, key, exactp,
			       &top->mp_ki);
	...
	if (data && (rc = mdb_read_data(cursor->mc_db, mpp.mp_page, leaf,
					data)) != MDB_SUCCESS)
		return rc;

	rc = mdb_set_key(cursor->mc_db, mpp.mp_page, leaf, key);
	if (rc == MDB_SUCCESS) {
		DPRINTF("==> cursor placed on key %.*s", (int)key->mv_size,
			(char *)key->mv_data);
		;
	}
...
static int mdb_cursor_first(MDB_cursor *cursor, MDB_val *key, MDB_val *data)
{
	...
	rc = mdb_search_page(cursor->mc_db, cursor->mc_txn, NULL, cursor, 0,
			     &mpp);
	if (rc != MDB_SUCCESS)
		return rc;
	assert(IS_LEAF(mpp.mp_page));
		   ...
		   MDB_cursor_op op)
{
	...
	case MDB_CURSOR_EXACT:
		while (CURSOR_TOP(cursor) != NULL)
			cursor_pop_page(cursor);
		if (key == NULL || key->mv_size == 0 ||
		    key->mv_size > MAXKEYSIZE) {
			rc = EINVAL;
...
static size_t mdb_branch_size(MDB_db *db, MDB_val *key)
{
	size_t sz;
...
static int mdb_add_node(MDB_db *db, MDB_page *mp, indx_t indx, MDB_val *key,
			MDB_val *data, pgno_t pgno, uint8_t flags)
{
	unsigned int i;
	...
	DPRINTF("add node [%.*s] to %s page %lu at index %i, key size %zu",
		key ? (int)key->mv_size : 0, key ? (char *)key->mv_data : NULL,
		IS_LEAF(mp) ? "leaf" : "branch", mp->mp_pgno, indx,
		key ? key->mv_size : 0);
	...
	if (IS_LEAF(mp)) {
		...
		if (F_ISSET(flags, F_BIGDATA)) {
			...
			node_size -= data->mv_size - sizeof(pgno_t);
			   ...
			   db->md_env->me_head.mh_psize / MDB_MINKEYS) {
			int ovpages = PAGEHDRSZ + data->mv_size +
				      db->md_env->me_head.mh_psize - 1;
			ovpages /= db->md_env->me_head.mh_psize;
			/* Put data on overflow page. */
			DPRINTF("data size is %zu, put on overflow page",
				data->mv_size);
			node_size -= data->mv_size - sizeof(pgno_t);
			if ((ofp = mdbenv_new_page(db->md_env, P_OVERFLOW,
						   ovpages)) == NULL)
				return MDB_FAIL;
	...
	if (node_size + sizeof(indx_t) > SIZELEFT(mp)) {
		...
		DPRINTF("upper - lower = %u - %u = %u", mp->mp_upper,
			mp->mp_lower, mp->mp_upper - mp->mp_lower);
...
static void mdb_del_node(MDB_db *db, MDB_page *mp, indx_t indx)
{
	unsigned int sz;
...
static int mdb_update_key(MDB_db *db, MDB_page *mp, indx_t indx, MDB_val *key)
{
	...
	node = NODEPTR(mp, indx);
	ptr = mp->mp_ptrs[indx];
	DPRINTF("update key %u (ofs %u) [%.*s] to [%.*s] on page %lu", indx,
		ptr, (int)node->mn_ksize, (char *)NODEKEY(node),
		(int)key->mv_size, (char *)key->mv_data, mp->mp_pgno);
			 ...
			 MDB_pageparent *dst, indx_t dstindx)
{
	...
	DPRINTF("moving %s node %u [%.*s] on page %lu to node %u on page %lu",
		IS_LEAF(src->mp_page) ? "leaf" : "branch", srcindx,
		(int)srcnode->mn_ksize, (char *)NODEKEY(srcnode),
		src->mp_page->mp_pgno, dstindx, dst->mp_page->mp_pgno);
	...
	rc = mdb_add_node(bt, dst->mp_page, dstindx, &key, &data,
	...
	if (srcindx == 0 && src->mp_pi != 0) {
		DPRINTF("update separator for source page %lu to [%.*s]",
			src->mp_page->mp_pgno, (int)key.mv_size,
			(char *)key.mv_data);
	...
	if (srcindx == 0 && IS_BRANCH(src->mp_page)) {
		MDB_val nullkey;
		nullkey.mv_size = 0;
		assert(mdb_update_key(bt, src->mp_page, 0, &nullkey) ==
		       MDB_SUCCESS);
	}

	if (dstindx == 0 && dst->mp_pi != 0) {
		DPRINTF("update separator for destination page %lu to [%.*s]",
			dst->mp_page->mp_pgno, (int)key.mv_size,
			(char *)key.mv_data);
	...
	if (dstindx == 0 && IS_BRANCH(dst->mp_page)) {
		MDB_val nullkey;
		nullkey.mv_size = 0;
		assert(mdb_update_key(bt, dst->mp_page, 0, &nullkey) ==
		       MDB_SUCCESS);
	}
...
static int mdb_merge(MDB_db *bt, MDB_pageparent *src, MDB_pageparent *dst)
{
	...
	DPRINTF("merging page %lu and %lu", src->mp_page->mp_pgno,
		dst->mp_page->mp_pgno);
	...
	DPRINTF("dst page %lu now has %lu keys (%.1f%% filled)",
		dst->mp_page->mp_pgno, NUMKEYS(dst->mp_page),
		(float)PAGEFILL(bt->md_env, dst->mp_page) / 10);
	...
	if (src->mp_pi == 0) {
		key.mv_size = 0;
		if ((rc = mdb_update_key(bt, src->mp_parent, 0, &key)) !=
		    MDB_SUCCESS)
			return rc;
	}
...
static int mdb_rebalance(MDB_db *db, MDB_pageparent *mpp)
{
	MDB_node *node;
	MDB_page *root;
	MDB_pageparent npp;
	...
	DPRINTF("rebalancing %s page %lu (has %lu keys, %.1f%% full)",
		...
		mpp->mp_page->mp_pgno, NUMKEYS(mpp->mp_page),
		(float)PAGEFILL(db->md_env, mpp->mp_page) / 10);
	...
	if (mpp->mp_parent == NULL) {
		...
		} else if (IS_BRANCH(mpp->mp_page) &&
			   NUMKEYS(mpp->mp_page) == 1) {
			DPRINTF("collapsing root page!");
			db->md_env->me_txn->mt_root =
				NODEPGNO(NODEPTR(mpp->mp_page, 0));
			if ((root = mdbenv_get_page(
				     db->md_env,
				     db->md_env->me_txn->mt_root)) == NULL)
				return MDB_FAIL;
			db->md_env->me_meta.mm_stat.ms_depth--;
	...
	if (mpp->mp_pi == 0) {
		...
		if ((npp.mp_page = mdbenv_get_page(db->md_env,
						   NODEPGNO(node))) == NULL)
			return MDB_FAIL;
	...
	DPRINTF("found neighbor page %lu (%lu keys, %.1f%% full)",
		npp.mp_page->mp_pgno, NUMKEYS(npp.mp_page),
		(float)PAGEFILL(db->md_env, npp.mp_page) / 10);
	 ...
	 */
	if (PAGEFILL(db->md_env, npp.mp_page) >= FILL_THRESHOLD &&
	    NUMKEYS(npp.mp_page) >= 2)
...
int mdb_del(MDB_db *bt, MDB_txn *txn, MDB_val *key, MDB_val *data)
{
	int rc, exact;
	unsigned int ki;
	MDB_node *leaf;
	...
	DPRINTF("========> delete key %.*s", (int)key->mv_size,
		(char *)key->mv_data);
		     ...
		     MDB_val *newkey, MDB_val *newdata, pgno_t newpgno)
{
	...
	if (mdp->h.md_parent == NULL) {
		...
		if (mdb_add_node(bt, &pdp->p, 0, NULL, NULL, mdp->p.mp_pgno,
				 0) != MDB_SUCCESS)
			return MDB_FAIL;
	} else {
	...
	DPRINTF("separator is [%.*s]", (int)sepkey.mv_size,
		(char *)sepkey.mv_data);

	/* Copy separator key to the parent.
	 */
	if (SIZELEFT(rdp->h.md_parent) < mdb_branch_size(bt, &sepkey)) {
		rc = mdb_split(bt, &rdp->h.md_parent,
			       (unsigned int *)&rdp->h.md_pi, &sepkey, NULL,
			       rdp->p.mp_pgno);
		...
		rc = mdb_add_node(bt, rdp->h.md_parent, rdp->h.md_pi, &sepkey,
				  NULL, rdp->p.mp_pgno, 0);
	}
	if (rc != MDB_SUCCESS) {
		free(copy);
		return MDB_FAIL;
...
int mdb_put(MDB_db *bt, MDB_txn *txn, MDB_val *key, MDB_val *data,
	    unsigned int flags)
{
	...
	DPRINTF("==> put key %.*s, size %zu, data size %zu", (int)key->mv_size,
		(char *)key->mv_data, key->mv_size, data->mv_size);
	...
	if (rc == MDB_SUCCESS) {
		...
		if (leaf && exact) {
			if (F_ISSET(flags, MDB_NOOVERWRITE)) {
				DPRINTF("duplicate key %.*s", (int)key->mv_size,
					(char *)key->mv_data);
				return EEXIST;
			}
			mdb_del_node(bt, mpp.mp_page, ki);
...
static pgno_t mdbenv_compact_tree(MDB_env *env, pgno_t pgno, MDB_env *envc)
{
	...
	if (F_ISSET(p->mp_flags, P_BRANCH)) {
		for (i = 0; i < NUMKEYS(p); i++) {
			node = NODEPTR(p, i);
			node->mn_pgno =
				mdbenv_compact_tree(env, node->mn_pgno, envc);
...
int mdbenv_get_flags(MDB_env *env, unsigned int *arg)
{
	if (!env || !arg)
		return EINVAL;
...
int mdbenv_get_path(MDB_env *env, const char **arg)
{
	if (!env || !arg)
		return EINVAL;
...
int mdbenv_stat(MDB_env *env, MDB_stat **arg)
{
...
int mdb_open(MDB_env *env, MDB_txn *txn, const char *name, unsigned int flags,
	     MDB_db **db)
{
	if (!name) {
		*db = (MDB_db *)&env->me_db;
